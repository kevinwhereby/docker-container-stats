...
app.post("/monitor", async (req, res) => {
    ...
    if (!body.labels) {
        res.status(400).send(JSON.stringify({ message: "labels property is required" }));
        return;
    }
    if (typeof body.labels !== "object") {
        res.status(400).send(JSON.stringify({ message: "labels property must be an object" }));
        return;
    }
    const labels = body.labels;
    if (Object.keys(labels).length === 0) {
        res.status(400).send(JSON.stringify({ message: "labels object must contain labels" }));
        return;
    }
    if (Object.values(labels).some((value) => typeof value !== "string" && typeof value !== null)) {
        res.status(400).send(JSON.stringify({ message: "label values must be string or null" }));
        return;
    }
    ...
    if (results.length > 1) {
        res.status(400).send(JSON.stringify({ message: "Found more than one container matching provided labels" }));
        return;
    }
    ...
    if (!container) {
        res.status(404).send(JSON.stringify({ message: "Successfully listed details but could not fetch container" }));
        return;
    }
    console.log("Monitoring", container.id);
    containerMonitor.monitorContainer(container);

    res.status(201).send(JSON.stringify({ containerId: containerDetails.Id }));
});

app.get("/monitor/:containerId", (req, res) => {
    const containerId = req.params.containerId;
    if (!containerId) {
        res.status(400).send(JSON.stringify({ message: "container ID is required" }));
        return;
    }
